# bayes_optimize_parallel_v5_0.py  (FIXED)
# - Explicit Expert/ExpertParameters writing to INI
# - Auto Symbol/Period from baseline .set name (FX_CarryMomentum_<SYMBOL>_<TF>.set)
# - New --expert arg (default FX_CarryMomentum.ex5)
# - Parallel over multiple MT5 terminals via --terms
# - Robust CSV seeding; no inline semicolons (fixed SyntaxError)
...
TF_SET = {'M1','M5','M15','M30','H1','H2','H3','H4','H6','H8','H12','D1','W1','MN1'}
...
def run_one(term_exe: str, template_text: str, base_set_path: str, expert: str, params: Dict,
    ...
    note = "NO_REPORT"
    metrics = {
        "ProfitFactor": 0.0, "MaxDDPercent": 0.0, "TotalTrades": 0,
        "NetProfit": 0.0, "GrossProfit": 0.0, "GrossLoss": 0.0, "Commissions": 0.0,
        "Swap": 0.0, "MaxDDMoney": 0.0, "SharpeRatio": 0.0
    }
...
def open_log_for_write(append: bool = False):
    ...
    if append and base_log.exists():
        ...
        return f, str(base_log), False
    try:
        f = open(base_log, "w", newline="", encoding="utf-8-sig")
        return f, str(base_log), True
    except PermissionError:
        ts = time.strftime("%Y%m%d_%H%M%S")
        alt = Path(RESULTS_DIR) / f"bayes_opt_log_{ts}.csv"
        f = open(alt, "w", newline="", encoding="utf-8-sig")
        print(f"[warn] {base_log} locked; using {alt}")
        return f, str(alt), True
    ...
    for log in logs:
        try:
            with open(log, "r", encoding="utf-8-sig") as f:
                ...
                for row in rdr:
                    if not row or len(row) <= score_idx:
                        continue
                    sc = parse_float_safe(row[score_idx])
                    if sc is None or sc <= -1e8:
                        continue
                    params = {}
...
def main():
    ...
    help=r"';' separated terminal64.exe paths OR 'auto' to scan C:\MT5_Portable")
    ...
    if args.terms.strip().lower() == "auto":
        ...
        if not terminals:
            raise SystemExit(f"[ERR] Auto-scan nenasel zadne terminal64.exe v {AUTO_BASE_DIR}")
        print("[info] Auto-detekovane terminaly:")
        for t in terminals: print("  -", t)
    else:
        terminals = [t for t in args.terms.split(";") if t.strip()]
    if not terminals:
        raise SystemExit("No terminals provided. Use --terms auto or explicit list pathA;pathB;pathC")

    ensure_dirs(terminals)
    kill_mt5_all()
                ...
                for fut in list(running.keys()):
                    if fut.done():
                        ...
                        try:
                            ...
                            print("[ERR] worker failed:", e)
                            completed += 1; done_any = True
                            continue
