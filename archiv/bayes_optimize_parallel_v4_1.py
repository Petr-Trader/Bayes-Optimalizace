# bayes_optimize_parallel_v4_1.py
import argparse
import csv
import json
import os
...
# ===================== DEFAULT PATHS =====================
TEMPLATE_INI   = r"C:\CLON_Git\Optimalizace\reports\bayes\manual_test.ini"
OUT_INI_DIR    = r"C:\CLON_Git\Optimalizace\reports\bayes\_jobs"
REL_REPORT_DIR = r"reports\bayes"
RESULTS_DIR    = r"C:\CLON_Git\Optimalizace\bayes\results"
...
def save_set(base_lines: List[str], overrides: Dict, out_path: str):
    out_lines = []
...
def score_from_metrics(m: Dict, obj: str, maxdd_cap=8.0, min_trades=0, target_trades=400, dd_lambda=0.2):
    ...
    if obj == "pf_trades":
        ...
        return (pf - penalty) * (0.5 + 0.5 * bonus)  # scale to [0.5,1)
...
def run_one(term_exe: str, template_text: str, base_set_path: str, params: Dict,
    ...
    ini_text = tweak_ini(template_text, {"ExpertParameters": set_name, "Report": report_rel})
    ...
    try:
        ...
        return {
            "slug": slug, "term": Path(term_exe).parent.name, "set_name": set_name,
            "report": "", "rc": -1, "secs": secs, "params": overrides,
            "metrics": {"ProfitFactor":0.0,"MaxDDPercent":0.0,"TotalTrades":0},
            "score": -1e9, "note": f"RUN_ERR:{e.__class__.__name__}"
        }
    secs = time.time() - start

    # --- Wait for report to appear (up to 300s) ---
    ...
    if Path(report_abs).exists():
        try:
            m = parse_mt5_html_metrics(report_abs)  # parser should return keys above if available
            for k in metrics.keys():
                if k in m and m[k] is not None:
                    metrics[k] = float(m[k]) if k not in ("TotalTrades",) else int(m[k])
            note = "PARSED"
        except Exception as e:
    ...
    return {
        "slug": slug, "term": Path(term_exe).parent.name, "set_name": set_name,
        "report": report_abs, "rc": 0, "secs": secs, "params": overrides,
        "metrics": metrics, "score": score, "note": note
    }

def detect_terminals_auto() -> List[str]:
    base = Path(AUTO_BASE_DIR)
...
def open_log_for_write(append: bool = False):
    base_log = Path(RESULTS_DIR) / "bayes_opt_log.csv"
    if append and base_log.exists():
        f = open(base_log, "a", newline="", encoding="utf-8-sig")
        return f, str(base_log), False  # False => do not write header
    try:
        f = open(base_log, "w", newline="", encoding="utf-8-sig"); return f, str(base_log), True
    except PermissionError:
        ts = time.strftime("%Y%m%d_%H%M%S"); alt = Path(RESULTS_DIR) / f"bayes_opt_log_{ts}.csv"
        f = open(alt, "w", newline="", encoding="utf-8-sig"); print(f"[warn] {base_log} locked; using {alt}"); return f, str(alt), True

def clamp_to_bounds(params: Dict[str, float], pbounds: Dict[str, Tuple[float, float]]) -> Dict[str, float]:
    out = {}
...
def iter_resume_logs(spec: str) -> List[Path]:
    """Accept: file, directory (take all *.csv), or wildcard; ';' separated."""
    import glob
    paths: List[Path] = []
    for part in spec.split(";"):
        ...
        if not part:
            continue
        # wildcard pattern anywhere (absolute or relative) -> glob.glob
        ...
        if p.exists():
            if p.is_dir():
                # all CSVs in dir
                paths.extend(sorted(p.glob("*.csv"), key=lambda t: t.stat().st_mtime))
            elif p.is_file() and p.suffix.lower() == ".csv":
                paths.append(p)
    return paths


def seed_from_logs(optimizer: BayesianOptimization, logs: List[Path], space: Dict, pbounds: Dict[str, Tuple[float, float]]) -> int:
    seeded = 0
    used = set()
    for log in logs:
        try:
            with open(log, "r", encoding="utf-8-sig") as f:
                ...
                try:
                    ...
                    # old header? try last known position 6
                    score_idx = 6 if len(header) > 6 and header[6] == "score" else None
                if score_idx is None: continue
                # param columns are those present in space.keys()
                param_indices = {k: header.index(k) for k in space.keys() if k in header}
                for row in rdr:
                    if not row or len(row) <= score_idx: continue
                    sc = parse_float_safe(row[score_idx])
                    if sc is None or sc <= -1e8:  # skip failed / filtered
                        continue
                    params = {}
                    ...
                    try:
                        optimizer.register(params=params, target=sc)
                        used.add(key)
                        seeded += 1
                    except Exception:
                        # some versions may forbid duplicates; ignore
                        pass
        except Exception:
            pass
    return seeded
...
def main():
    ...
    ap.add_argument("--space", default=r"C:\CLON_Git\Optimalizace\bayes\space.json")
    ap.add_argument("--set", dest="base_set", default=r"C:\MT5_Portable\MT5_testA\MQL5\Profiles\Tester\STO_ADX_v1.02.set")
    ...
    ap.add_argument("--dd_lambda", type=float, default=0.2, help="Penalty weight for netdd objective")
    # Early-stop
    ap.add_argument("--patience", type=int, default=120, help="Stop if no improvement for N completed evals (after min_evals)")
    ap.add_argument("--tol", type=float, default=0.01, help="Relative improvement needed (e.g. 0.01 = +1%)")
    ap.add_argument("--min_evals", type=int, default=200, help="Don't early-stop before this many evals")
    # Resume
    ap.add_argument("--resume_from_log", default="", help="CSV file/dir/wildcard; ';' separated allowed. Seeds optimizer from previous runs.")
    ap.add_argument("--append_log", action="store_true", help="Append to existing results\\bayes_opt_log.csv (no header).")
    args = ap.parse_args()

    if args.terms.strip().lower() == "auto":
        terminals = detect_terminals_auto()
        if not terminals: raise SystemExit(f"[ERR] Auto-scan nenašel žádné terminal64.exe v {AUTO_BASE_DIR}")
        print("[info] Auto-detekované terminály:"); [print("  -", t) for t in terminals]
    else:
        terminals = [t for t in args.terms.split(";") if t.strip()]
    if not terminals: raise SystemExit("No terminals provided. Use --terms auto nebo explicitní seznam pathA;pathB;pathC")

    ensure_dirs(terminals); kill_mt5_all()

    template_text = read_text(TEMPLATE_INI)
    ...
    Path(RESULTS_DIR).mkdir(parents=True, exist_ok=True)
    f, log_path, write_header = open_log_for_write(append=args.append_log and not args.resume_from_log)
    best_score = -1e18
    stagnation = 0

    # If we seeded, try to set best_score accordingly
    try:
    ...
    with f:
        ...
        def cast_all(params_floats: Dict) -> Dict:
            casted = {}
            for k, v in params_floats.items(): casted[k] = cast_param(k, v, space[k])
            return casted

        total_needed = args.init_points + args.iters
        submitted = 0; completed = 0; running = {}

        def submit_point(executor, params_floats: Dict, tag: str):
            nonlocal submitted
            if submitted >= total_needed: return False
            casted = cast_all(params_floats)
            term = terminals[submitted % len(terminals)]
            fut = executor.submit(run_one, term, template_text, args.base_set, casted, REL_REPORT_DIR,
        ...
        with ThreadPoolExecutor(max_workers=len(terminals)) as ex:
            # bootstrap init points (if we already seeded a lot, pořád dává smysl pár náhodných)
            ...
            while completed < total_needed:
                ...
                for fut in list(running.keys()):
                    if fut.done():
                        ...
                        # early-stop check
                        if res["score"] > best_score * (1.0 + args.tol):
                            best_score = res["score"]; stagnation = 0
                        else:
                            stagnation += 1

                        # keep pipeline full (unless early-stop imminent)
                        ...
                        if still_need:
                            if submitted < args.init_points:
                                ...
                                try:
                                    ...
                                    sug = {k: random.uniform(lo, hi) for k, (lo, hi) in pbounds.items()}
                                submit_point(ex, sug, "SUG")

                        done_any = True

                # Early stop: if stagnation exceeded patience and we have enough evals
    ...
    best = optimizer.max
    (Path(RESULTS_DIR) / "bayes_opt_best.json").write_text(json.dumps(best, indent=2), encoding="utf-8")
    print("Best:", best); print("Log:", log_path); print(f"Seeded from logs: {seeded}")
